...
def datingClassTest():
    ...
    # 从文件中加载数据
    datingDataMat, datingLabels = file2matrix('E:/2.KNN/2.KNN/datingTestSet2.txt')  # load data setfrom file
    # 归一化数据
...
def handwritingClassTest():
    # 1. 导入数据
    hwLabels = []
    trainingFileList = listdir('E:/2.KNN/2.KNN/trainingDigits')  # load the training set
    ...
    for i in range(m):
        ...
        trainingMat[i, :] = img2vector('E:/2.KNN/2.KNN/trainingDigits/%s' % fileNameStr)

    # 2. 导入测试数据
    testFileList = listdir('E:/2.KNN/2.KNN/testDigits')  # iterate through the test set
    errorCount = 0.0
    mTest = len(testFileList)
    for i in range(mTest):
        fileNameStr = testFileList[i]  # 得到文件名
        fileStr = fileNameStr.split('.')[0]  # take off .txt
        classNumStr = int(fileStr.split('_')[0])  # 得到classNum
        vectorUnderTest = img2vector('E:/2.KNN/2.KNN/testDigits/%s' % fileNameStr)
